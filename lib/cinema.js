...
export function montarFaixas(filmes, { agora = new Date() } = {}) {
  ...
  const faixas = [
    ...
    // "Nacional" é o áudio que a Ingresso.com usa para o filme falado em
    // português — não há campo de país no catálogo, e esta é a marca mais
    // próxima disso que a fonte dá.
    { chave: 'nacional', titulo: 'Produções nacionais',
      filmes: filmes.filter((f) => (f.audios || []).includes('Nacional')) },
